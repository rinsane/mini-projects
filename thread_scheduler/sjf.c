...
void sjf_scheduler(ThreadInfo threads[], pthread_t thread_ids[], int n) {
    int completed = 0;

    while (completed < n) {
        int idx = -1;
        int min_cpu_time = __INT_MAX__;

        // Find the shortest job that has arrived
        for (int i = 0; i < n; i++) {
            if (threads[i].arrival_time <= global_time && threads[i].cpu_time > 0 && threads[i].cpu_time < min_cpu_time) {
                min_cpu_time = threads[i].cpu_time;
                idx = i;
            }
        }

        // If no job is found, increment global_time (CPU IDLE)
        if (idx == -1) {
        ...
        pthread_mutex_lock(&mutex);
        current_thread = threads[idx].id;  // Allow the current thread to run
        ...
        threads[idx].cpu_time = 0;  // Mark this thread as completed
        completed++;
    }
}
...
int main() {
    ...
    sjf_scheduler(threads, thread_ids, n);
